...
static std::unique_ptr<project> import_and_cook_resources()
{
  ...
  auto clip_look_left_additive{std::make_unique<clip_additive_uncooked>("look_-45_additive")};
  ...
  auto clip_look_right_additive{std::make_unique<clip_additive_uncooked>("look_45_additive")};
  ...
  auto graph{std::make_unique<anim_graph_uncooked>("graph")};
...
void app_example_additive::update(const float dt_s)
{
  ...
  ImGui::SetNextWindowSize(ImVec2(340.0F, 100.0F));
  ...
  if (ImGui::Begin(
    ...
    ImGui::SliderFloat("Playback speed", &playback_speed_value, 0.0F, 2.0F, "%.1f");
