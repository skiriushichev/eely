...
static void ImDrawList_AddBezierWithArrows(ImDrawList* drawList, const ImCubicBezierPoints& curve, float thickness,
    float startArrowSize, float startArrowWidth, float endArrowSize, float endArrowWidth, float midArrowSize, float midArrowWidth,
    bool fill, ImU32 color, float strokeThickness, const ImVec2* startDirHint = nullptr, const ImVec2* endDirHint = nullptr)
{
    ...
    if (fill)
    {
        ...
        if (midArrowSize > 0.0f)
        {
            const ImVec2 sample_0 = ImCubicBezierSample(curve, 0.5f);
            const ImVec2 sample_1 = ImCubicBezierSample(curve, 0.6f);
            
            const auto mid_dir = ImNormalized(ImVec2(sample_1.x - sample_0.x, sample_1.y - sample_0.y));
            const auto mid_n = ImVec2(-mid_dir.y, mid_dir.x);
            const auto half_width = midArrowWidth * 0.5f;
            const auto tip        = sample_0 + mid_dir * midArrowWidth;

            drawList->PathLineTo(sample_0 + mid_n * ImMax(half_width, half_thickness));
            drawList->PathLineTo(sample_0 - mid_n * ImMax(half_width, half_thickness));
            drawList->PathLineTo(tip);
            drawList->PathFillConvex(color);
        }
    ...
    else
    {
        ...
        if (midArrowSize > 0.0f)
        {
            ...
            if (half_width > half_thickness)
                drawList->PathLineTo(sample_0 + mid_n * half_width);
            drawList->PathLineTo(tip);
            if (half_width > half_thickness)
                drawList->PathLineTo(sample_0 - mid_n * half_width);
        }
...
void ed::Link::Draw(ImDrawList* drawList, ImU32 color, float extraThickness) const
{
    ...
    ImDrawList_AddBezierWithArrows(drawList, curve, m_Thickness + extraThickness,
        ...
        m_StartPin && m_StartPin->m_ArrowWidth > 0.0f ? m_StartPin->m_ArrowWidth + extraThickness : 0.0f,
          ...
          m_MidArrowSize, m_MidArrowWidth,
        true, color, 1.0f,
...
bool ed::EditorContext::DoLink(LinkId id, PinId startPinId, PinId endPinId, ImU32 color, float thickness)
{
    ...
    link->m_IsLive        = true;
    link->m_MidArrowSize = GetStyle().LinkMidArrowSize;
    link->m_MidArrowWidth = GetStyle().LinkMidArrowWidth;
...
float* ed::Style::GetVarFloatAddr(StyleVar idx)
{
    switch (idx)
    {
        ...
        case StyleVar_LinkMidArrowSize:         return &LinkMidArrowSize;
        case StyleVar_LinkMidArrowWidth:        return &LinkMidArrowWidth;
