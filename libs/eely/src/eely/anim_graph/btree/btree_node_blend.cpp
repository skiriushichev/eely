...
btree_node_blend::btree_node_blend(bit_reader& reader) : btree_node_base(reader)
{
  ...
  const gsl::index param_values_size{reader.read(bits_anim_graph_node_index)};
...
void btree_node_blend::serialize(bit_writer& writer) const
{
  ...
  writer.write(
      {.value = gsl::narrow<uint32_t>(param_values_size), .size_bits = bits_anim_graph_node_index});
...
std::unique_ptr<anim_graph_node_base> btree_node_blend::clone() const
{
